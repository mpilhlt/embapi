...
func TestPaginationForGetUsers(t *testing.T) {
	...
	// Parse the response as UserBrief array (direct array, not wrapped)
	...
	assert.NoError(t, err)
	assert.Equal(t, 1, len(users), "Expected exactly 1 user with limit=1")
	...
	assert.GreaterOrEqual(t, len(users), 1, "Expected at least 1 user")
