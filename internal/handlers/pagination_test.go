...
// Parse the response as UserBrief array
type GetUsersResponseBody struct {
	Users []struct {
		UserHandle          string `json:"user_handle"`
		Name                string `json:"name,omitempty"`
		NumberOfProjects    int    `json:"number_of_projects"`
		NumberOfDefinitions int    `json:"number_of_definitions"`
		NumberOfInstances   int    `json:"number_of_instances"`
	} `json:"users"`
}
var responseBody GetUsersResponseBody
err = json.Unmarshal(bodyBytes, &responseBody)
assert.NoError(t, err)
assert.Equal(t, 1, len(responseBody.Users), "Expected exactly 1 user with limit=1")
...
assert.GreaterOrEqual(t, len(responseBody.Users), 1, "Expected at least 1 user")

fmt.Printf("All users (limit=100): %d users found\n", len(responseBody.Users))
